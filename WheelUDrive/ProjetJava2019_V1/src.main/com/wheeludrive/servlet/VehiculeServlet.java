...
public class VehiculeServlet extends AbstractWheelUDriveServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (sendMail != null) {
			...
			if (message.matches("^[<>={}]*$")) {
				request.setAttribute(EMAIL_PROCESS_ERROR,
						"Contenu du message non valide. N'utilisez pas de caract̬res speciaux. (Les caract̬res <, >, =, {, }) sont interdit");
			...
			try {
				...
				log.debug("Valeur passee en param̬tre : " + request.getParameter("id_voiture"));
				e.printStackTrace();
				request.setAttribute(EMAIL_PROCESS_ERROR,
			...
			try {
				...
				log.debug(e.getMessage());
				log.debug("Valeur passee en param̬tre : " + request.getParameter("id_vendeur"));
				e.printStackTrace();
				request.setAttribute(EMAIL_PROCESS_ERROR,
						"Une erreur s'est produite lors de l'envoi de l'email. Veuillez reessayer plus tard.");
				request.getRequestDispatcher(VUE).forward(request, response);
